...
plt.savefig(WORK_DIR / "lavaan_comparison/poldem_optim.png")
plt.close()
...
pd.DataFrame({"est": est.detach(), "se": se.detach()}).to_csv(WORK_DIR / "lavaan_comparison/poldem_pars.csv")

### HOLZINGER-SWINEFORD MODEL ###
# The holzinger-swineford model
opt_hs = ts.SemOptions.from_file(WORK_DIR / "lavaan_comparison/hs_mod.pkl")
df_hs = pd.read_csv(WORK_DIR / "lavaan_comparison/hs.csv")[opt_hs.ov_names]  # order the columns, important step!
...
plt.savefig(WORK_DIR / "lavaan_comparison/hs_optim.png")
plt.close()

# write parameter estimates and se
est = mod_hs.free_params
se = mod_hs.Inverse_Hessian(ts.mvn_negloglik(dat_hs, mod_hs())).diag().sqrt()
pd.DataFrame({"est": est.detach(), "se": se.detach()}).to_csv(WORK_DIR / "lavaan_comparison/hs_pars.csv")
