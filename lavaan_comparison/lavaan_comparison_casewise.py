...
optim_pol = torch.optim.Adam(mod_pol.parameters(), lr = LRATE, betas=(0.999, 0.9))
ll_pol = []
...
plt.savefig(WORK_DIR / "stochastic_optim.png")
plt.close()

est = mod_pol.free_params
se = mod_pol.Inverse_Hessian(ts.mvn_negloglik(dat_pol, mod_pol())).diag().sqrt()
pd.DataFrame({"est": est.detach(), "se": se.detach()}).to_csv(WORK_DIR / "poldem_pars_casewise.csv")
