...
df = pd.read_csv(WORK_DIR / "med_dat.csv")[opt.ov_names]  # order the columns, important step!
...
se = torch.zeros(est.shape)  # no standard errors because they take a long time
pd.DataFrame({"est": est.detach(), "se": se.detach()}).to_csv(WORK_DIR / "params_uls.csv")


### LASSO ESTIMATION ###
mod_lasso = ts.StructuralEquationModel(opt)
mod_lasso.load_state_dict(copy.deepcopy(mod_uls.state_dict()))  # init from ULS estimates
...
for epoch in range(MAXIT):
    ...
    paths = torch.cat((mod_lasso.B_0[1000,:-2], mod_lasso.B_0[:-2, 1001]))
